...
function get-dbApp {
	...
	$query = "SELECT * FROM Programas WHERE name LIKE '%$name%'" #Consulta para obtener los todos los datos de las app
	...
	if ($null -eq $data) {
		wError("No se encontró ningun dato en la base de datos para $name")
		Pause
		exit
	}
	$appNum = 1
	if ($data -is [Object] -and $data.Count -ge 2) {
		$apps = @()
		...
		Winfo("Se han encontrado 2 elementos con el nombre '$name'")
		$apps | Select-Object @{label = "No"; expression = { $_.App } }, ID, @{ label = "Aplicación"; expression = { $_.name } } | Out-Host
		$options = ($apps | ForEach-Object { $_.App })
		$AppSel = Get-UserChoice -text ("Escribe el número de la aplicación que deseas usar: ") -Options $options
		$apps | ForEach-Object {
			if ([string]$_.App -ieq [string]$AppSel) {
				return get-dbAppDataByID($_.ID)
			}
		}
...
function get-dbAppDataByID {
	Param(
		[Parameter(Mandatory = $false)]
		[int]$appID = ''
	)
	#si se desean ver las aplicaciones en la base de datos escribir get-dbAllData sin ningun parametro
	...
	#Continuación luego de verificar el envío de datos
	$query = "SELECT * FROM Programas WHERE id=$appID" #Consulta para obtener los todos los datos de las app

	#Si se solicita una app se debe devolver solo el objeto que contiene los datos, no un arreglo con los mismos
	$data = Get-DbQuery($query)
	if ($null -eq $data) {
		wError("No se encontró ningun dato en la base de datos para $appID")
		Pause
		exit
	}
	if ($data -ne '') {
